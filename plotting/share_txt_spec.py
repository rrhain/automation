...
class MyDlvd(std_plot):
    ...
    xcol = 'N'
    ycols = ['GGen']
    ydata = ['(Density == 5) & (DutyCycle == 10)', '(Density == 10) & (DutyCycle == 10)']
    options=['linestyle="dashed"']

class MyDlvdvCR(std_plot):
    title = 'Delivery Ratio vs Data Rate for Duty Cycle 10%'
    xcol = 'Data_Rate'
    ycols = ['Recv']
...
class MyDlvdvMob(std_plot):
    title = 'Delivery Ratio vs Mobility Model'
    xcol = 'Error_Rate'
    ycols = ['Recv']
    ydata = ['(Contention_Range == 250) & (DutyCycle == 10)', '(Contention_Range == 500) & (Duty_Cycle == 10)']
    legend = ['CR=250', 'CR=500']
    options=['linestyle="dashed"']
